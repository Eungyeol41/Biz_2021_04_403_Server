...
public class TodoCommandImplV1 implements TodoCommand{
	...
	/*
	 * Logger 객체인 log를 선언하고 생성을 할 때 관리 이름으로 TODO를 부착하라
	 * TODO라는 이름으로 Logger를 싱클톤으로 생성하라
	 * 
	 * Factory 패턴
	 * 	객체를 생성하는 클래스.method()를 준비해두고 필요에 따라 생성된 객체를 제공받는 패턴
	 */
	...
	public void execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		 * value를 Object로 만든 인 이유
		 * table에서 데이터를 SELECT하거나, INSERT, UPDATE를 수행할 때 각 칼럼의 Data type이 문자열, 숫자 등 다양한 type으로 구성되어 있기 때문에
		 * 	미리 어떤 Type으로 지정하기가 어려워 Super parent type인 Object 클래스 type으로 선언한다
		 */
