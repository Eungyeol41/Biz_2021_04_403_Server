 ...
 * 공통으로 처리해야 하는 것들을
 * 
 *  미리 Filter에서 처리를 하고 처리된 결과를 Controller에게 전달하는 Tomcat WAS의 처리 절차
 *  
 *  Filter에서 처리된 설정은 모든 Controller에 일일이 설정하는 것과 같은 효과를 발휘한다
 *  
 *  login과 같은 처리는 각각의 Controller가 사용자의 요청을 받으면 항상 login이 되어있는 지 검사하고 그에 따른 처리를 수행해야 하는데 
 *  
...
/*
 * Filter에서 response.setContentType()을 지정할 때 text/html로 지정을 하면 jsp, js, 기타 여러가지 static 파일들이 모두 html ContentType으로 지정되어 
 * 	WebBrowser로 전송되어 제 기능을 수행하지 못하는 현상이 발생한다
 * 
 * 이럴 때는 WebFilter urlPatterns를 문자열 배열로 생성하고 각 Controller에 지정된 WebServlet 필터를 나열해주면 
 * 	Controller로 요청되는 사항들만 Filtering을 하게되어 기타 다른 static File들에는 영향을 미치지 않는다.
 */
