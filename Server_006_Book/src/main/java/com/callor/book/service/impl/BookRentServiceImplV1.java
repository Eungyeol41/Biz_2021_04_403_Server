...
public class BookRentServiceImplV1 implements BookRentService{
	 ...
	 * 2. 떠넘기기(던지기)
	 * 		method throws를 추가하여 호출한 method에게 exception을 되돌려보내기
	 * 		현재 method에서 직접 exception을 처리하는 코드가 없어서 코드가 다소 간소화된다
	 * 
	...
	public BookRentDTO findById(Long seq) {
		 ...
		 * 만약 매개변수로 직접 SQL 명령문을 다음과 같이 만들면 " WHERE 도서명 = " + title;  
		 * title 변수값에 1 OR 1 = 1과 같은 문자열을 담아서 보내면 실제 쿼리는 WHERE 도서명 = 1 OR 1 = 1과 같이 만들어져 버린다.
		 * 
		 * Prepare... 를 사용할 때는 조건문의 변수가 포함될 위치에 ? 기호를 사용한다
		 * WHERE 도서명 = ?와 같이 사용한다
		 */
